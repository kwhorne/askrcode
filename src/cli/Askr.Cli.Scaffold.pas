...
procedure NyttProsjekt(const ParentDir, Name: string;
  WithAuth: Boolean = False);
procedure LagModell(const Rot, Name: string; WithMigration: Boolean);
...
var
  ...
  I: Integer;
begin
  Path_ := IncludeTrailingPathDelimiter(Rot) + Folder;
  Name_ := TStringList.Create;
  try
    Name_.Sorted := True;
    if FindFirst(IncludeTrailingPathDelimiter(Path_) + Prefiks + '*.pas',
      faAnyFile, R) = 0 then
    begin
      repeat
    ...
    Emit(IncludeTrailingPathDelimiter(Path_) + IndeksUnit + '.pas', Source_);
  finally
    Name_.Free;
  end;
end;
...
procedure NyttProsjekt(const ParentDir, Name: string;
  WithAuth: Boolean);
var
  Rot, Framework, LaufDep, Pin: string;
begin
  Rot := IncludeTrailingPathDelimiter(ParentDir) + Name;
  ...
  Emit(Rot + '/app/Http/App.Http.HomeController.pas',
  ...
  Emit(Rot + '/frontend/package.json',
    '{' + #10 +
  ...
  Emit(Rot + '/frontend/vite.config.js',
  ...
  Emit(Rot + '/frontend/src/Layout.svelte',
    '<script>' + #10 +
  ...
  Emit(Rot + '/frontend/src/pages/Home.svelte',
    '<script>' + #10 +
...
begin
  N := PascalName(Name);
  Table_ := Flertall(SnakeName(N));

  Emit(IncludeTrailingPathDelimiter(Rot) + 'app/Models/App.Models.' +
    N + '.pas',
  ...
  if WithMigration then
    LagMigrasjon(Rot, 'Create' + PascalName(Table_));
end;
...
begin
  N := PascalName(Name);
  Path_ := Flertall(SnakeName(N));
  Emit(IncludeTrailingPathDelimiter(Rot) + 'app/Http/App.Http.' +
    N + 'Controller.pas',
...
begin
  N := PascalName(Name);
  Emit(IncludeTrailingPathDelimiter(Rot) + 'database/App.Seeders.' +
    N + '.pas',
...
begin
  N := PascalName(Name);
  Name_ := SnakeName(N);
  Emit(IncludeTrailingPathDelimiter(Rot) + 'app/Jobs/App.Jobs.' + N + '.pas',
